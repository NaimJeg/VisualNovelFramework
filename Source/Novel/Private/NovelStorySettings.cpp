...
#if WITH_EDITOR
EDataValidationResult UNovelStorySettings::IsDataValid(FDataValidationContext& Context) const
{
    EDataValidationResult Result = Super::IsDataValid(Context);
    bool bHasError = Result == EDataValidationResult::Invalid;

    if (RootScreenClass.IsNull())
    {
        Context.AddError(FText::FromString(TEXT("RootScreenClass is required.")));
        bHasError = true;
    }

    if (DialogueScreenWidgetClass.IsNull())
    {
        Context.AddError(FText::FromString(TEXT("DialogueScreenWidgetClass is required.")));
        bHasError = true;
    }

    if (!ChapterMap.Contains(0))
    {
